...
:func:`Tree.render` method or interactively visualized using a
built-in Graphical User Interface (GUI) invoked by the
:func:`Tree.show` method.
...
node (tree or subtree), you can simply call the :func:`Tree.show`
method.
...
To generate an image, the :func:`Tree.render` method should be
used instead of :func:`Tree.show`. The only required argument is
...
can be passed to the :func:`Tree.show` and :func:`Tree.render`
...
::
  ...
  from ete4 import Tree, TreeStyle, TextFace
  t = Tree( "((a,b),c);" )
  ts = TreeStyle()
  ts.show_leaf_name = True
...
instance, :func:`Tree.copy` will replicate all node styles in the
...
.. currentmodule:: ete4.treeview.faces
...
Several types of node faces are provided by the main :mod:`ete4`
...
found at the :mod:`ete4.treeview` reference page..
...
:func:`Tree.add_face` method. For instance, if two text labels
