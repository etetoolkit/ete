...
class Test_Coretype_Tree(unittest.TestCase):
    ...
    def test_tree_manipulation(self):
        ...
        t = Tree(nw2_full)
        self.assertTrue(t.get_leaf_names(), [name for name in  t.iter_leaf_names()])
        self.assertTrue(t.get_leaves(), [name for name in  t.iter_leaves()])
        self.assertTrue(t.get_descendants(), [n for n in  t.iter_descendants()])
